...
This document covers several modes of use for the TLA⁺ tools, outside of the common GUI-based [TLA⁺ VS Code extension](github.com/tlaplus/vscode-tlaplus/).
...
It is safe to run TLC concurrently from multiple separate processes.
...
These all involve running Java on `tla2tools.jar` using whatever command line invocation utilities your language possesses, then capturing the process's output.
...
The command will exit with a nonzero error code if parsing failed.
